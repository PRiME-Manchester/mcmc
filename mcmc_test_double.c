...
double gaussPdf(double x, double s);
void pcg_test();
void app_done();
...
void pcg_test(void)
{
  ...
  // // Memory allocation
  // nums = (uint32_t *)sark_xalloc (sv->sdram_heap, RAND_NUMS*sizeof(uint32_t), 0, ALLOC_LOCK);
  // if (!nums)
  // {
  ...
  for(i=r_min; i<r_max; i++)
  {
    for(j=0; j<THIN; j++)
    {
      ...
      x = gammaPdf(r, 3.0, 1.0/(y*y*4));
      y = 1.0/(x+1) + gaussPdf(r, 1.0/sqrt(2*x+2));
      //io_printf(IO_BUF, "%u\n", num);
    }
...
double gaussPdf(double x, double s)
{
   return 1/s/M_SQRT1_2 * exp(-(x-s)*(x-s)/(2*s*s));
