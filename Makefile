...
#  LIBRARY := -L$(LIB_DIR) -lsark
  ...
  LD := $(GP)-gcc -T$(SCRIPT) -Wl,--start-group -lgcc -lc -lm -lrdimon -Wl,--end-group -Wl,-e,cpu_reset -Wl,--gc-sections -Wl,--use-blx 
...
# Added -lm switch to include math libraries
# which results in a .aplx file of 21KB!

$(APP).elf: $(OBJECTS) $(SCRIPT) $(LIBRARY) 
	mkbuild $(APP) > sark_build.c
	$(CC) sark_build.c
	$(LD) $(LFLAGS) $(OBJECTS) sark_build.o $(LIBRARY) -o $(APP).elf -lm
